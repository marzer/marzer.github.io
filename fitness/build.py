...
LIFT_ORDER = [
    'goblet_squat', 'floor_press', 'row', 'rdl', 'reverse_lunge', 'ohp',
    'pullover', 'chin_ups', 'hip_thrust',
]
LIFT_NAMES = {
...
def parse_sets(text, where):
    ...
    for tok in str(text).split():
        ...
        if not m:
            fail(f"{where}: bad set token '{tok}' (expected REPSxWEIGHT, e.g. '12x20' or '8xbw')")
...
class Plot:
    ...
    def frame(self, y_fmt=lambda v: f'{v:g}'):
        ...
        for t in y_ticks(self.y0, self.y1):
            yy = self.y(t)
            parts.append(f'<line class="grid" x1="{self.ml}" y1="{yy:.1f}" x2="{self.w - self.mr}" y2="{yy:.1f}"/>')
            parts.append(f'<text class="ticklabel" x="{self.ml - 6}" y="{yy + 3.5:.1f}" text-anchor="end">{y_fmt(t)}</text>')
        base = self.mt + self.ph
        parts.append(f'<line class="axis" x1="{self.ml}" y1="{base:.1f}" x2="{self.w - self.mr}" y2="{base:.1f}"/>')
        span = (self.d1 - self.d0).days
        for d in date_ticks(self.d0, self.d1):
            xx = self.x(d)
            parts.append(f'<text class="ticklabel" x="{xx:.1f}" y="{base + 16:.1f}" text-anchor="middle">{tick_label(d, span)}</text>')
        return ''.join(parts)
...
def end_marker(plot, d, v, label, cls='dot-end'):
    ...
    return (f'<circle class="dot-ring" cx="{x:.1f}" cy="{y:.1f}" r="6.5"/>'
...
def start_marker(p, start):
    ...
    return (f'<rect class="pre-band" x="{p.ml}" y="{p.mt}" width="{x - p.ml:.1f}" height="{p.ph}"/>'
...
def chart_figure(title, inner, table_rows, table_head, empty=False):
    ...
    if table_rows:
        ...
        body = ''.join('<tr>' + ''.join(f'<td class="num">{esc(c)}</td>' for c in row) + '</tr>'
                       for row in table_rows)
        table = (f'<details><summary>data</summary><table>'
...
def weight_chart(weights, config):
    ...
    body.append(f'<rect class="goal-band" x="{p.ml}" y="{band_top:.1f}" '
                f'width="{p.pw}" height="{band_bot - band_top:.1f}"/>')
    body.append(f'<text class="bandlabel" x="{p.w - p.mr - 6}" y="{band_top + 14:.1f}" '
                f'text-anchor="end">goal {goal_lo:g}–{goal_hi:g}</text>')
    for d, v in points:
        body.append(f'<circle class="dot-raw" cx="{p.x(d):.1f}" cy="{p.y(v):.1f}" r="2.5">'
                    f'<title>{fmt_date(d)}: {v:.1f} kg</title></circle>')
    body.append(polyline(p, trend, 'line-data'))
    td, tv = trend[-1]
    body.append(end_marker(p, td, tv, f'{tv:.1f}'))
    return chart_figure('Weight (kg) — all weigh-ins, 28-day trend',
                        svg(W, H, ''.join(body)), rows, ('date', 'kg'))


def running_chart(garmin_runs, manual_runs, config):
    knee_by_date = {r['date']: r.get('knee') for r in manual_runs if r.get('knee') is not None}
    ...
    rows = [(fmt_date(wk), f'{mins:.0f}') for wk, mins in sorted(weeks.items(), reverse=True)]
    if not weeks or len(run_dates) < 2:
        return chart_figure('Running — weekly minutes, knee response', '', rows, ('week of', 'min'), empty=True)
    ...
    for wk, mins in sorted(weeks.items()):
        ...
        h = p.mt + p.ph - y
        if h > 0.5:
            body.append(f'<path class="bar" d="{bar_path(cx - bw / 2, y, bw, h)}">'
                        f'<title>week of {fmt_date(wk)}: {mins:.0f} min</title></path>')
        if wk == latest_wk and mins > 0:
            body.append(f'<text class="endlabel" x="{cx:.1f}" y="{y - 6:.1f}" '
                        f'text-anchor="middle">{mins:.0f} min</text>')
    strip_top = 10
    level_y = lambda score: strip_top + (3 - score) * ((strip_h - 20) / 3) + 6
    for score in (0, 1, 2, 3):
        yy = level_y(score)
        body.append(f'<text class="ticklabel" x="{p.ml - 6}" y="{yy + 3.5:.1f}" text-anchor="end">{score}</text>')
    body.append(f'<text class="striplabel" x="{p.ml}" y="{strip_top - 1}">knee score, morning after (0 = silent, 3 = bad)</text>')
    for d in run_dates:
        score = knee_by_date.get(d)
        if score is None:
            continue
        x, y = p.x(d), level_y(score)
        body.append(f'<circle class="dot-ring" cx="{x:.1f}" cy="{y:.1f}" r="6.5"/>'
                    f'<circle class="knee k{score}" cx="{x:.1f}" cy="{y:.1f}" r="4.5">'
                    f'<title>{fmt_date(d)}: knee {score}</title></circle>')
    return chart_figure('Running — weekly minutes, knee response',
                        svg(W, H, ''.join(body)), rows, ('week of', 'min'))
...
def lift_charts(strength):
    ...
    for key in order:
        ...
        if len(vals) < 2:
            ...
            inner = (f'<div class="chart-empty">{fmt_kg(last) if last is not None else "no data"}'
                     f'{" " + unit if last is not None else ""} — need 2+ sessions to chart</div>')
            panels.append(chart_figure(lift_name(key), inner, rows, ('date', 'top set')))
            continue
        ...
        for d, v in vals[:-1]:
            body.append(f'<circle class="dot-mid" cx="{p.x(d):.1f}" cy="{p.y(v):.1f}" r="3">'
        ...
        panels.append(chart_figure(lift_name(key), svg(W, H, ''.join(body)), rows, ('date', 'top set')))
    return '<div class="multiples">' + ''.join(panels) + '</div>'


def simple_line_chart(title, points, unit, y_fmt=lambda v: f'{v:g}', decimals=1, trend=False, start=None):
...
def build_tiles(garmin, strength, manual_runs, measures):
    ...
    if weights:
        ...
        tiles.append(tile('Weight, 28-day trend', f'{cur_v:.1f}<span class="unit">kg</span>',
                          delta_span(rate, good_when_down=True, decimals=2, unit=' kg/wk',
                                     vs=f'rate, last {span} days'), hero=True))
    else:
    ...
    if measures:
        ...
        if waists:
            ...
            tiles.append(tile('Waist', f'{waists[-1][1]:g}<span class="unit">cm</span>',
                              delta_span(delta, good_when_down=True, unit=' cm', vs='vs previous')))
    daily = garmin['daily']
    rhr = [(r['date'], r['resting_hr']) for r in daily if r.get('resting_hr')]
    if rhr:
        last7 = [v for d, v in rhr if d > rhr[-1][0] - timedelta(days=7)]
        prev7 = [v for d, v in rhr if rhr[-1][0] - timedelta(days=14) < d <= rhr[-1][0] - timedelta(days=7)]
        cur = sum(last7) / len(last7)
        delta = (cur - sum(prev7) / len(prev7)) if prev7 else None
        tiles.append(tile('Resting HR, 7-day', f'{cur:.0f}<span class="unit">bpm</span>',
    ...
    if steps:
        ...
        cur = sum(last7) / len(last7)
        tiles.append(tile('Steps, 7-day avg', f'{cur / 1000:.1f}<span class="unit">k</span>'))
    ...
    mins = sum(r['duration_s'] / 60 for r in garmin['runs'] if week_monday(r['date']) == this_wk)
    prev = sum(r['duration_s'] / 60 for r in garmin['runs'] if week_monday(r['date']) == this_wk - timedelta(days=7))
    if garmin['runs']:
        tiles.append(tile('Run minutes this week', f'{mins:.0f}<span class="unit">min</span>',
                          delta_span(mins - prev if prev else None, good_when_down=False, decimals=0, unit=' min')))
    return '<div class="tiles">' + ''.join(tiles) + '</div>'


def page(title, body, depth, generated):
    up = '../' * depth
    nav = (f'<nav><a class="brand" href="{up if depth else "./"}">marzer / <strong>fitness</strong></a>'
...
def render_dashboard(garmin, strength, manual_runs, measures, notes, config):
    ...
    rhr = [(r['date'], float(r['resting_hr'])) for r in garmin['daily'] if r.get('resting_hr')]
    parts.append(simple_line_chart('Resting HR (bpm) — 7-day trend', rhr, 'bpm', decimals=0,
                                   trend=True, start=config['start']))
    if notes:
        latest = notes[-1]
        parts.append(f'<h2>Latest note — {esc(latest["label"])}</h2>'
...
def render_log(garmin, strength, manual_runs, measures, notes):
    ...
    months = sorted({(d.year, d.month)
                     for d in ([e['date'] for e in strength] + all_run_dates +
                               [m['date'] for m in measures] + [n['date'] for n in notes])},
                    reverse=True)
    if not months:
    ...
    for y, mo in months:
        ...
        for n in reversed(month_notes):
            parts.append(f'<h3>{esc(n["label"])}</h3><div class="note">{n["html"]}</div>')
        ...
        if month_runs:
            ...
            for d in month_runs:
                ...
                pace = fmt_pace(g['duration_s'] / (g['distance_m'] / 1000)) if g.get('distance_m') else '—'
                ...
                rows.append('<tr>'
                            ...
                            '</tr>')
            parts.append('<h3>Runs</h3><table><thead><tr><th>date</th><th>rung</th><th>time</th>'
        ...
        if month_strength:
            ...
            for e in reversed(month_strength):
                lifts = '<br>'.join(f'{esc(lift_name(k))}: {esc(fmt_sets(s))}'
                                    for k, s in e['lifts'].items())
                note = f'<div class="cell-note">{esc(e["notes"])}</div>' if e['notes'] else ''
                rows.append(f'<tr><td>{fmt_date(e["date"])}</td>'
                            f'<td class="num">{esc(e["session"])}</td><td>{lifts}{note}</td></tr>')
            parts.append('<h3>Strength</h3><table><thead><tr><th>date</th><th>session</th>'
        ...
        if month_measures:
            ...
            for m in reversed(month_measures):
                ...
                rows.append(f'<tr><td>{fmt_date(m["date"])}</td><td class="num">{esc(vals)}</td></tr>')
            parts.append('<h3>Measurements</h3><table><tbody>' + ''.join(rows) + '</tbody></table>')
    return ''.join(parts)


def render_program(program_path):
    text = program_path.read_text(encoding='utf-8')
    return '<div class="prose">' + markdown.markdown(text, extensions=['tables']) + '</div>'
...
def main():
    ...
    (out / 'index.html').write_text(
        page('fitness — marzer', render_dashboard(garmin, strength, manual_runs, measures, notes, config), 0, generated),
        encoding='utf-8')
    (out / 'log' / 'index.html').write_text(
        page('log — fitness — marzer', render_log(garmin, strength, manual_runs, measures, notes), 1, generated),
        encoding='utf-8')
    (out / 'program' / 'index.html').write_text(
        page('program — fitness — marzer', render_program(src / 'program.md'), 1, generated),
        encoding='utf-8')
    shutil.copy2(src / 'static' / 'fitness.css', out / 'fitness.css')

    counts = (f'{len(garmin["weights"])} weigh-ins, {len(garmin["runs"])} runs, '
